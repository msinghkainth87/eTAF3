...
public class AssertionMethods extends GetElementUsingBy implements BaseTest {
	...
	public void checkElementPartialText(String accessType, String actualValue, String accessName, boolean testCase) throws TestCaseFailed {
		...
		if (testCase) {
			if (!elementText.contains(actualValue))
				throw new TestCaseFailed("Text does not contain expected string\nExpected:" + actualValue + "\nActual:" + elementText);
		} else {
			if (elementText.contains(actualValue))
				throw new TestCaseFailed("Text does contain expected string\nExpected:" + actualValue + "\nActual:" + elementText);
		}
	}
	...
	/**
	 * Is option from radio button group selected.
	 *
	 * @param accessType       : String : Locator type (id, name, class, xpath, css)
	 * @param by               : String : by value attribute or innerText
	 * @param option           : String : the expected value
	 * @param accessName       : String : the actual object locator string for id,name,class,xpath,css
	 * @param shouldBeSelected the should be selected
	 * @throws TestCaseFailed the test case failed
	 */
//method to assert option from radio button group is selected/unselected
	public void isOptionFromRadioButtonGroupSelected(String accessType, String by, String option, String accessName, boolean shouldBeSelected) throws TestCaseFailed {
		List<WebElement> radioButtonGroup = wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(getElementByAttributes(accessType, accessName)));
	...
	public void isOptionFromDropdownSelected(String accessType, String by, String option, String accessName, boolean shouldBeSelected) throws TestCaseFailed {
		...
		WebElement dropdown = wait.until(ExpectedConditions.presenceOfElementLocated(getElementByAttributes(accessType, accessName)));
	...
	/**
	 * Method to validate any element
	 *
	 * @param accessType : String : Locator type (id, name, class, xpath, css)
	 * @param value      : String : Variable args for Name of by type and String Option to select in that order
	 * @param accessName : String : Locator value
	 */
	public Boolean validateElement(String accessType, String accessName, String value) {
		...
		switch (elementType.toLowerCase()) {
			...
			case "select":
				//getOptionFromDropdown(accessType, "text", value, accessName);
				break;
			case "checkbox":
				//return isCheckboxChecked(accessType, accessName)
			case "a":

			default:
				//Checkbox(accessType, accessName);		//nothing more than an element click. Didn't wanna import ClickElementMethods class just for this.
				break;
		}
		return true;
	}
